...
`client_id` is your application's `client_id`. `scope` is a list of [OAuth2 scopes](/docs/mezon-topics/oauth2#shared-resources-oauth2-scopes) separated by url encoded spaces (`%20`). `redirect_uri` is whatever URL you registered when creating your application, url-encoded. `state` is the unique string mentioned in [State and Security](/docs/mezon-topics/oauth2#state-and-security).
...
`code` is now exchanged for the user's access token by making a `POST` request to the [token URL](/docs/mezon-topics/oauth2#shared-resources-oauth2-urls) with the following parameters:
